...
<main class="flex-1 overflow-x-hidden">
  ...
  <?php bg_reveal_start(); ?>
  <section class="liquid-stage mx-auto grid max-w-6xl gap-10 px-4 py-14 lg:grid-cols-2">
    <div class="relative z-10">
      <h2 class="font-display text-xl font-semibold text-white"><?php echo h(bg_pick(array('zh' => '联系方式', 'en' => 'Contact Info'), $lang)); ?></h2>
      <dl class="mt-6 space-y-5">
        <?php foreach ($rows as $i => $row): ?>
        <div class="glass-panel flex items-start gap-4 p-4">
          <span class="font-mono text-xs text-tech-blue/80">0<?php echo $i + 1; ?></span>
          <div>
            <dt class="text-sm font-medium text-white"><?php echo h($row['dt']); ?></dt>
            <dd class="mt-1 text-sm text-white/75"><?php echo h($row['dd']); ?></dd>
          </div>
        </div>
        <?php endforeach; ?>
      </dl>
      <div class="mt-8">
        ...
        <p class="mt-3 text-sm text-white/75"><?php echo h(implode('、', $cities)); ?> <?php echo h(bg_pick($UI['footer']['etc'], $lang)); ?></p>
      </div>
    </div>

    <div class="glass-panel relative overflow-hidden p-6 sm:p-8" data-contact>
      <div class="pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full bg-tech-blue/25 blur-3xl"></div>
      <!-- 成功态 -->
      <div class="hidden h-full flex-col items-center justify-center text-center" data-contact-success>
        ...
        <p class="mt-4 text-lg font-semibold text-white"><?php echo h(bg_pick(array('zh' => '提交成功', 'en' => 'Submitted'), $lang)); ?></p>
        <p class="mt-2 text-sm text-white/75"><?php echo h(bg_pick(array('zh' => '感谢您的留言，我们会尽快与您联系。', 'en' => "Thank you for your message. We'll be in touch soon."), $lang)); ?></p>
        <button type="button" data-contact-reset class="glass-btn mt-6 px-5 py-2 text-sm"><?php echo h(bg_pick(array('zh' => '再填一份', 'en' => 'Submit Another'), $lang)); ?></button>
      </div>
      <!-- 表单态 -->
      <form class="relative space-y-4" data-contact-form
        ...
        <div>
          <label for="name" class="text-sm font-medium text-white"><?php echo h(bg_pick(array('zh' => '姓名', 'en' => 'Name'), $lang)); ?> <span class="text-red-400">*</span></label>
        ...
        <div>
          <label for="phone" class="text-sm font-medium text-white"><?php echo h(bg_pick(array('zh' => '电话', 'en' => 'Phone'), $lang)); ?> <span class="text-red-400">*</span></label>
        ...
        <div>
          <label for="message" class="text-sm font-medium text-white"><?php echo h(bg_pick(array('zh' => '需求说明', 'en' => 'Project Details'), $lang)); ?></label>
        ...
        <p class="hidden rounded-lg bg-red-500/15 px-3 py-2 text-sm text-red-200" data-contact-error></p>
        <button type="submit" data-contact-submit class="glass-btn w-full px-6 py-3 text-sm">
          <span data-contact-btn-text><?php echo h(bg_pick(array('zh' => '提交咨询', 'en' => 'Submit'), $lang)); ?></span>
        </button>
      </form>
    </div>
  </section>
