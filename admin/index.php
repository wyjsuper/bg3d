...
<div class="mx-auto max-w-5xl">
  <div class="glass-panel p-6 md:p-8">
    ...
    <h1 class="mt-2 text-2xl font-bold text-white md:text-3xl">欢迎回来 👋</h1>
    <p class="mt-2 max-w-2xl text-sm leading-relaxed text-white/75">这里是北港3D设计官网的统一内容管理中心。在左侧选择模块即可维护网站全部内容，修改后会实时生效于前台页面。</p>
  </div>

  <h2 class="mb-4 mt-8 flex items-center gap-2 text-sm font-semibold text-white"><?php echo bg_admin_icon('layout-dashboard', 16); ?>内容模块</h2>
    ...
    <?php foreach ($BG_COLLECTIONS as $c): ?>
      ...
      <a href="<?php e(bg_url('/admin/content/' . $c['type'])); ?>" class="glass-panel group flex items-center gap-4 p-5 transition-all hover:-translate-y-0.5 hover:border-tech-blue/40 hover:shadow-lg hover:shadow-tech-blue/10">
        ...
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm font-semibold text-white"><?php e($c['label']); ?></p>
          <p class="text-xs text-white/60"><?php echo !empty($c['singleton']) ? '单条设置' : h($c['singular']) . '管理'; ?></p>
        </div>
  ...
  <div class="glass-panel mt-8 border-dashed p-5 text-xs leading-relaxed text-white/60">
